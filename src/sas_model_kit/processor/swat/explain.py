...
class BatchExplainProcessor(
    BaseBatchProcessor[DataFrame | SASDataFrame, CASTable, CASTable]
):
    """Batch processor for Explain executions (CSRP).

    Generic parameters:
        - ResourceT: DataFrame | SASDataFrame (Operation input)
        - UploadReturnT: CASTable (Operation.upload_data() return)
        - BatchReturnT: CASTable (process_batch() return - final concatenated table)
    ...
    def __init__(
        ...
        *,
        ...
        """
        # Initialize base class (sets self.operation)
        super().__init__(operation)

        # Initialize Final dependencies
        self._parameter = parameter
        self._transformer = ShapleyValuesTransformer(id_column=id_column)
        self._batch_executor = BatchOperationExecutor(operation=operation)
    ...
    def process_batch(
        ...
        **context: Any,
        ...
        Returns:
            Scheme A semantics:
            - All success: Ok(final_castable)
            - Partial success: Err with partial_output (final_ref dict)
        ...
        Example:
            ...
            ... )
            >>> if result.is_ok:
            ...     cas_table = result.unwrap()
            ...     print(f"Results: {cas_table.params}")
        """
        ...
        try:
            ...
            final_castable = self._process_concat(
                cas_tables, batch_caslib, output_table
            )

            # Cleanup temp tables
            if cleanup_temp_tables and cas_tables:
                self.batch_executor.cleanup_tables(cas_tables)  # type: ignore

            if errors:
                return self._err(
                    "BatchExplainProcessor completed with some errors",
                    errors=errors,
                    partial_output=final_castable,
                )
            if final_castable is None:
                return self._err(
                    "BatchExplainProcessor failed: no results generated",
                    errors=errors,
                    partial_output=None,
                )

            return self._ok(final_castable)
    ...
    def _process_concat(
        ...
        """Concatenate list of CASTable into final output table.

        Args:
            cas_tables: List of CASTable to concatenate
            batch_caslib: CAS library for output table
            output_table: Output table name

        Returns:
            Final concatenated CASTable
        """
        if not cas_tables:
            return None
        ...
        final_castable = cast(
            CASTable,
            self.batch_executor.concat_tables(
                inputs=cas_tables,  # type: ignore
                caslib=batch_caslib,
                output_table=output_table,
            ),
        )
        return final_castable
        ...
        **context: Any,
        ...
        for id_ in ids:
            try:
                ...
                yield ExecutionSuccess(id=id_, data=cas_table)
