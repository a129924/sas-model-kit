...
try:
    ...
    raise ImportError(
        "SWAT is required for SWATConnection. "
        "Install it with: pip install swat"
    )


class SWATConnection:
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._kwargs = kwargs
        self._session: Optional[swat.CAS] = None
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to connect to CAS server "
                f"{self.hostname}:{self.port}: {e}"
            ) from e
    ...
    def close(self) -> None:
        ...
        try:
            ...
            raise ConnectionError(
                f"Failed to close CAS session: {e}"
            ) from e
        finally:
            self._session = None
    ...
    def get_session(self) -> swat.CAS:
        ...
        if not self.is_healthy():
            raise ConnectionError(
                "Connection is not healthy. Call reconnect()."
            )
